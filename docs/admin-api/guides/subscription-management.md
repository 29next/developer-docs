...
Updating subscription recurring items and pricing can be done through [subscriptionLinesCreate](/docs/admin-api/reference/#/operations/subscriptionsLinesCreate), [subscriptionLinesUpdate](/docs/admin-api/reference/#/operations/subscriptionsLinesUpdate), and [subscriptionsLinesDestroy](/docs/admin-api/reference/#/operations/subscriptionsLinesDestroy) endpoints.

**Adding an Additional Product**

To add a new product to a subscription, use the [subscriptionLinesCreate](/docs/admin-api/reference/#/operations/subscriptionsLinesCreate) Admin API endpoint with the product, price, and quanity details.
...
To update and existing product price and quantity on a subscription line, use the [subscriptionsLinesUpdate](/docs/admin-api/reference/#/operations/subscriptionsLinesUpdate) Admin API endpoint with the new price, and new quanity details.
...
To remove a product from a subscription, send a DELETE request to the [subscriptionsLinesDestroy](/docs/admin-api/reference/#/operations/subscriptionsLinesDestroy) endpoint to remove the line item (ie the product) from future renewal orders created from the subscription.
...
Changing the next renewal date of a subscription can be achieved through updating the `next_renewal_date` key on the subscription object with a PATCH request to the [subscriptionsPartialUpdate](/docs/admin-api/reference/#/operations/subscriptionsPartialUpdate) endpoint with your new renewal date and time.
...
To change the payment gateway used for bankcard payments of a subscription, send a PATCH request to the [subscriptionsPartialUpdate](/docs/admin-api/reference/#/operations/subscriptionsPartialUpdate) endpoint with the new `payment_gateway`.
...
Subscriptions that are `past_due` status can be attemted to retry the renewal, often combined with a new `payment_gateway`, by using the [subscriptionsRetryCreate](/docs/admin-api/reference/#/operations/subscriptionsRetryCreate) endpoint.
