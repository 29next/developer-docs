...
from api_enums import (
    CAMPAIGNS_API_SPEC_SOURCE, CAMPAIGNS_API_DESCRIPTION, CAMPAIGNS_API_SPEC_ADDITIONS,
    ADMIN_API_SPEC_SOURCE, ADMIN_API_DESCRIPTION, ADMIN_API_SPEC_ADDITIONS
)
...
def download_file(type, source, version):
    destination = BASE_OUTPUT_PATH + '/{}/{}.yaml'.format(type, version)
    params = {
        'version': version
    }
...
def update_spec_file(type, source, version, description, additions):
...
def update_api_spec():
    for version in API_VERSIONS:
        ...
        download_file(
            version['type'], version['source'], version['version']
        )
        update_spec_file(
            version['type'], version['source'], version['version'], version['description'],
            version['additions'],
        )
