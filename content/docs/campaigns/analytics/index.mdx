...
import { Settings, Sparkles, Activity, Bug, CheckCircle, BookOpen } from 'lucide-react';
...
<Tabs items={['Simple API', 'Advanced API']}>
  <Tab value="Simple API">
...
<Tabs items={['Configuration', 'Runtime']}>
...
1. **Configure providers** - Set up GTM, Facebook Pixel, or other platforms
   ...
   See [Examples Overview](/docs/campaigns/analytics/examples)

2. **Learn tracking methods** - Understand the tracking API

   See [Tracking API Reference](/docs/campaigns/analytics/tracking-api)

3. **Review events** - See all standard e-commerce events

   See [Event Reference](/docs/campaigns/analytics/events)

4. **Advanced tracking** - Create custom events and use transform functions

   See [Custom Events](/docs/campaigns/analytics/custom-events)

## Documentation Structure

<Cards>
  <Card
    href="/docs/campaigns/analytics/configuration"
    icon={<div className="size-9 rounded-md flex items-center justify-center bg-blue-500/15 text-blue-500"><Settings className="size-4" /></div>}
    title="Configuration"
    description="Detailed analytics configuration options — modes, providers, and debug settings."
  />
  <Card
    href="/docs/campaigns/analytics/tracking-api"
    icon={<div className="size-9 rounded-md flex items-center justify-center bg-green-500/15 text-green-500"><Activity className="size-4" /></div>}
    title="Tracking API"
    description="Complete tracking API — trackAddToCart, trackPurchase, trackViewItem, and more."
  />
  <Card
    href="/docs/campaigns/analytics/custom-events"
    icon={<div className="size-9 rounded-md flex items-center justify-center bg-orange-500/15 text-orange-500"><Sparkles className="size-4" /></div>}
    title="Custom Events"
    description="Create custom events and transform functions for advanced tracking scenarios."
  />
  <Card
    href="/docs/campaigns/analytics/examples"
    icon={<div className="size-9 rounded-md flex items-center justify-center bg-purple-500/15 text-purple-500"><BookOpen className="size-4" /></div>}
    title="Examples"
    description="Provider setup guides for GTM, Facebook Pixel, GA4, RudderStack, and custom webhooks."
  />
  <Card
    href="/docs/campaigns/analytics/debugging"
    ...
    title="Debugging"
    description="Troubleshoot analytics — debug mode, event inspection, and provider status checks."
  />
  <Card
    href="/docs/campaigns/analytics/best-practices"
    icon={<div className="size-9 rounded-md flex items-center justify-center bg-teal-500/15 text-teal-500"><CheckCircle className="size-4" /></div>}
    title="Best Practices"
    description="Implementation patterns and tips for reliable analytics across all providers."
  />
</Cards>
