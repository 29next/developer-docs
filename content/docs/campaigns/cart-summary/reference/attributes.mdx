...
Unified discount list container. Place inside the cart summary template to render discount rows. Accepts an optional filter value:
...
When placed inside a `data-summary-lines` row template, renders per-line discounts. The offer/voucher filter is not available at line level — all line discounts are rendered regardless of the filter value.

See [Cart Lines & Discounts → Discount Lists](../summary-lists#discount-lists) for usage examples.
