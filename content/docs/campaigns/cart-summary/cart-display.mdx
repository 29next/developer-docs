...
Use `CartDisplayEnhancer` when you need individual values scattered across the page — a running total in a sticky header, an item count badge, or a shipping cost in a shipping banner. Use [`CartSummaryEnhancer`](./get-started) when you need a grouped totals block.
