...
The Next Commerce JS SDK emits events that you can listen to for tracking and custom functionality. The SDK has 35 internal events that can be listened to via `next.on()`.
...
Fired when the order has been successfully created, just before the SDK redirects to the success URL:
...
<Callout type="info">
  After this event fires, the SDK automatically clears the cart and checkout state (items, vouchers, shipping method, form data, payment token, billing address) and then navigates to the resolved success URL. If you need to read cart or voucher data for analytics or custom cleanup, do it inside the `order:completed` handler — the stores are reset immediately afterward.
</Callout>

### `order:redirect-missing`

Fired when the order was created but no redirect URL could be resolved (no `payment_complete_url`, no `os-next-page` meta tag, no `order_status_url`, and the fallback confirmation URL was rejected):

```javascript
next.on('order:redirect-missing', ({ order }) => {
  // Cart and checkout state are NOT auto-reset in this branch, so the
  // merchant can retry or recover. Clear manually after your own redirect:
  window.location.href = `/thank-you?ref_id=${order.ref_id}`;
});
```
...
Here are all 35 available events organized by category:
...
- `order:redirect-missing` - Order created but no redirect URL resolved
...
if (window.location.search.includes('debug=true')) {
  const allEvents = [
    ...
    // Checkout & Order
    'checkout:started', 'checkout:form-initialized', 'order:completed', 'order:redirect-missing',
    // Payment
