...
```html
<span data-next-toggle-display="packageId"></span>             <!-- package ref_id -->
<span data-next-toggle-display="name"></span>                  <!-- package display name -->
<img  data-next-toggle-display="image" />                      <!-- package image (sets src on <img>) -->
<span data-next-toggle-display="quantity"></span>              <!-- package quantity -->
<span data-next-toggle-display="variantName"></span>           <!-- product variant name -->
<span data-next-toggle-display="productName"></span>           <!-- product name -->
<span data-next-toggle-display="sku"></span>                   <!-- product SKU -->
...
<span data-next-toggle-display="originalRecurringPrice"></span><!-- original recurring price before discounts -->
...
Boolean fields (`hasDiscount`, `isRecurring`, `isSelected`) show or hide the element via `display` — they do not write text. The `image` field sets `src` on `<img>` elements (and `alt` if not already set). All other fields set `textContent`. Unrecognized values leave the element unchanged.
...
## Discount Lists Inside Toggle Cards

Use `data-next-discounts` inside a toggle card to render one row per discount applied to the package. Each container needs a `<template>` child that defines the markup for a single discount row.
...
<Callout type="info">
Per-package discounts come from the per-line breakdown in the price calculation. The offer/voucher filter (`data-next-discounts="offer"` / `="voucher"`) is not available at toggle level — per-line discounts from the API are not split by type. Use the unfiltered `data-next-discounts` attribute.
</Callout>
