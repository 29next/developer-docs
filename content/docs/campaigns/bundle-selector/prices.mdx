...
## Discount Lists Inside Bundle Cards

Use `data-next-discounts` inside a bundle card to render one row per discount applied to the bundle. Each container needs a `<template>` child that defines the markup for a single discount row.

```html
<div data-next-bundle-card data-next-bundle-id="premium" ...>
  <span data-next-bundle-display="price"></span>

  <!-- All discounts (offers + vouchers) -->
  <div data-next-discounts>
    <template>
      <span class="badge badge-secondary">{discount.name}</span>
    </template>
  </div>
</div>
```

Filter by discount type using the attribute value:
...
```html
<!-- Separate lists for offers and vouchers -->
<div data-next-discounts="offer">
  <template><span class="badge">{discount.name}: -{discount.amount}</span></template>
</div>
<div data-next-discounts="voucher">
  <template><span class="badge">Coupon: {discount.name}</span></template>
</div>
```
...
Discount lists are populated after each bundle price fetch. Before the first fetch, the containers are empty and carry the `next-discounts-empty` class.
