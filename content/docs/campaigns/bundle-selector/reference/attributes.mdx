...
| `data-next-shipping-id` | Shipping method `ref_id` to auto-apply when this bundle is selected in swap mode |
...
Place inside a bundle card to render one row per discount applied to the bundle. Each container needs a `<template>` child defining the row markup.

| Value | Renders |
|-------|---------|
| *(empty or omitted)* | All discounts (offers + vouchers combined) |
| `"offer"` | Only offer/promotion discounts |
| `"voucher"` | Only voucher/coupon discounts |
...
Populated after each bundle price fetch. Before the first fetch, the containers are empty and carry the `next-discounts-empty` class.

When placed inside a slot template (`data-next-bundle-slot-template-id`), renders per-slot discounts. The offer/voucher filter is not available at slot level — all slot discounts are rendered regardless of the filter value.

See [Displaying Prices → Discount Lists](../prices#discount-lists-inside-bundle-cards) and [Slot Templates → Per-Slot Discount List](../slots#per-slot-discount-list) for usage examples.
