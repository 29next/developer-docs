...
```json title="Create Subscription" http-method="POST" http-target="https://{store}.29next.store/api/admin/subscriptions/"
{
...
```json title="Adding an Additional Product" http-method="POST" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/lines/"
{
...
```json title="Updating an Existing Product" http-method="PUT" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/lines/{lineId}/"
{
...
```json title="Remove an Existing Product" http-method="DELETE" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/lines/{lineId}/"
{}
```
...
```json title="Update Subscription Renewal Schedule" http-method="PATCH" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/"
{
    "interval": "day",
    "interval_count": 30,
...
```json title="Update Next Renewal Date" http-method="PATCH" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/"
{
...
```json title="Trigger Subscription Renewal" http-method="POST" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/renew/"
{}
```
...
```json title="Update Payment Gateway" http-method="PATCH" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/"
{
    "payment_details": {
...
```json title="New Bankcard Payment Method" http-method="PATCH" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/"
{
    "payment_details": {
...
```json title="Retry Subscription Renewal" http-method="POST" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/retry/"
{
...
```json title="Cancel Subscription" http-method="POST" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/cancel/"
{
...
For operations that affect a cohort of subscriptions — pausing recurring billing, shifting renewal dates, cancelling a churn-risk segment, or migrating subscriptions between payment gateways — iterate the [subscriptionsPartialUpdate](/docs/admin-api/reference/subscriptions/subscriptionsPartialUpdate) endpoint over your list of subscription IDs. Respect the **4 requests/second rate limit** (sleep ~0.26s between calls) and write the result of each PATCH to a log file so failures can be retried without re-running the whole batch.

**Pausing recurring billing for a cohort.** Shift each subscription's `next_renewal_date` out by a fixed offset (commonly 30–45 days). The status stays `active` and no cancellation email is sent — renewals are simply deferred.

```json title="Soft-Pause Subscription" http-method="PATCH" http-target="https://{store}.29next.store/api/admin/subscriptions/{id}/"
{
    "next_renewal_date": "2026-08-17T10:09:01-04:00" // current renewal + 45 days
}
```
