...
Create your webhook after the app is installed using the [webhooksCreate](/docs/admin-api/reference/webhooksCreate) Admin API and pass the `transaction.created, order.created, order.updated` to subscribe to these events.
...
When disputes are identified as belonging to a store, the dispute service should create a new dispute using the [disputesCreate](/docs/admin-api/reference/disputesCreate) Admin API.


#### Step 5 - Dispute service matches dispute to transaction

Disputes created in the store need to be matched to the transaction in the store to associate it with the order/customer. Disputes can be matched by passing the `transaction` parameter when creating or with update using the [disputesUpdate](/docs/admin-api/reference/disputesUpdate) Admin API. See example below in [Match a Dispute](#matching-disputes).
...
Depending on the type of dispute, the dispute service may need to create a refund using the [transactionsRefundCreate](/docs/admin-api/reference/transactionsRefundCreate) Admin API to resolve the dispute. See [Create a Refund](#creating-disputes) detail below.
...
Once the dispute is resolved, the dispute service should set the dispute resolution using the [disputesUpdate](/docs/admin-api/reference/disputesUpdate) Admin API. See [Dispute Resolutions](#dispute-resolutions) and [Resolve a Dispute](#resolving-disputes) detail below.


### Creating Disputes

To create a dispute in the store using the [disputesCreate](/docs/admin-api/reference/disputesCreate) Admin API, see example below:
...
To match a dispute to a transaction, pass the `transaction` id to the [disputesUpdate](/docs/admin-api/reference/disputesUpdate) API. Matching the dispute to the transaction will match the associated order and customer.
...
To create a refund for a transaction as part of the dispute resolution process, you can use the [transactionsRefundCreate](/docs/admin-api/reference/transactionsRefundCreate) Admin API.
...
<Callout type="info">
To retrieve a list of all fulfillment orders and their status, use the [ordersFulfillmentOrdersRetrieve](/docs/admin-api/reference/ordersFulfillmentOrdersRetrieve) endpoint.
</Callout>
<Tabs>
<TabItem value="fulfillment_status-unfulfilled" label="Fulfillment Status - Unfulfilled">

If order `fulfilmlent_status` is `unfulfilled`, your dispute service can stop fulfillment using the [fulfillmentOrdersHold](/docs/admin-api/reference/fulfillmentOrdersHold) endpoint.
...
If order `fulfilmlent_status` is `processing` your dispute service can request processing fulfillment orders be canceled with the fulfillment locations with the [cancellationRequestSend](/docs/admin-api/reference/cancellationRequestSend) endpoint.
...
RDR Alerts are automatically refunded with the gateway, dispute services should log an external refund for the transaction using the [transactionsRefundCreate](/docs/admin-api/reference/transactionsRefundCreate) Admin API.
