...
1. Retrieve available locations for items to move using the [availableLocationsRetrieve](/docs/admin-api/reference/fulfillment/availableLocationsRetrieve) endpoint.
2. Move fulfilllment order line items to a new location using the  [fulfillmentOrdersMove](/docs/admin-api/reference/fulfillment/fulfillmentOrdersMove) endpoint.
...
```json title="Retrieve Available Fulfillment Locations" http-method="GET" http-target="https://{store}.29next.store/api/admin/fulfillment-orders/{id}/available-locations/" 
{
    "available_locations": [
        {
            "id": 123,
...
```json title="Move Fulfillment Order Items to New Location" http-method="POST" http-target="https://{store}.29next.store/api/admin/fulfillment-orders/{id}/move/"
{
  "new_location_id": 123,
